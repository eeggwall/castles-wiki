...
|---|---|---|---|
| 1 every sign is a character | `(T±P)/2`, roots of unity, skyline DFT, mod-`p` period | characters of `Z/m` and `Z/w`; the cyclic group `<x>` | E, S, N |
| 2 the parity clause is not a character | the even-block indicator on `{1..h}^w` | `Z_h^w` and the Fourier spectrum of `(-1)^blocks` | E, X, F |
| 3 the full commutant of `M_k` | the `JD` involution and the sector factorization of `char_k` | `Z/2` or `Z/4` generated by `JD`, or something larger | N, E, Z |
| 4 cyclic sieving at `q = -1` | Hardin's `2^L` and the missing sign-reversing involution | `Z/n` rotation on towers via noncrossing partitions | N, Q, E |
| 5 the castle monoid and its abelianization | Klarner `A = P/(1-P)` and the sign as a monoid character | free monoid on primes; its abelianization | Q, E, Z |
| 6 kernel and cokernel of the boundary map | cycle rank `=` 2x2 blocks; Laplacian isospectrality | `H_1 = Z^{#2x2}`; the sandpile group `K(G)` | S, R, Z |
| 7 the identity element of the castle sandpile | the spectrum as the only shape invariant S uses | recurrent configurations under addition-and-stabilization | S, Z |
| 8 the ladder lives in a unit group | metallic means and the reachable-field census | `O_K^* = Z/2 x Z^r` (Dirichlet) | N, F |
| 9 the castle ring's unit group | Pohlig-Hellman, mod-`p` periods, Kitamasa, linear complexity | `prod Z/(p^{d_i} - 1)`, plus a `p`-group at repeated factors | X, N, F |
| 10 Burnside on the census; the order knob at the wall | mirror-deduping, "modulo cyclic shift", `Delta^alpha Delta^beta = Delta^{alpha+beta}` | `Z/2`, `Z/w`, the non-abelian `D_w`; `(R,+)` and where the homomorphism breaks | Z, S, R, F |
