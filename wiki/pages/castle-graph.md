...
The castle graph turns each Axis 9 predicate on [[castle-classification](pages/castle-classification.md)] into a graph property. Types this wiki now tracks:
...
The tall-column values within a tree castle are independent of the constraint, so counting *tree castles with area `A`* is the same as counting up-to-column-position which columns are short, then distributing the area over tall columns. The `q`-analogue (area-graded) sits inside the wiki's Q-Division.
...
- [[castle-classification](pages/castle-classification.md)] - Axis 9 is spectral predicates on this graph.
...
- [[castle-classification](pages/castle-classification.md)] Axis 8 - the tree-castle golden growth constant sits on rung 1 of the metallic ladder for height 2.
