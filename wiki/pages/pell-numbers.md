...
The working note that seeded this thread read the two atoms as PE 502's per-column binary state and the rule-3 gap, and called `D(x)` the `k = 1` baby case of the signed recurrence `den_k = den_{k−1}(1 − 2x) + x·num_{k−1}` on [[castle-counting-formula](pages/castle-counting-formula.md)]. Neither holds: height-2 castles under the actual rules number `2^w` (rule 3 is automatic in the column-height model), and `den_1 = 1 − 2x + 2x²`, the A146559 denominator. The correction and the computation behind it are on [[pell-castle-strip](pages/pell-castle-strip.md)].[^3][^8]
...
- [[pe502-pell-castle-strip](pages/pe502-pell-castle-strip.md)] — introduces the two-atom reading of `1/(1 − 2x − x²)`; its identification of the atoms with PE 502's rules is withdrawn on [[pell-castle-strip](pages/pell-castle-strip.md)]; the sequence identity was verified against OEIS A000129 during ingest.
...
- [[castle-counting-formula](pages/castle-counting-formula.md)] — the signed recurrence `den_k = den_{k−1}(1−2x) + x·num_{k−1}`; its `k = 1` member is `1 − 2x + 2x²` (A146559), not the Pell denominator.
...
[^8]: Verified by execution (Python 3, SymPy) on 2026-09-19, recorded in full on [[pell-castle-strip](pages/pell-castle-strip.md)] footnotes 4 and 6: skylines over `{1,2}` under the castle rules number `2, 4, 8, 16, 32, 64, 128` for `w = 1..7`; the `num_k/den_k` recurrence gives `den_1 = 1 − 2x + 2x²`; and for the 1-smooth matrix `M = [[1,1,0],[1,1,1],[0,1,1]]` on heights `{1,2,3}`, `e_1ᵀ(I − xM)^{−1}𝟙 = 1/(1 − 2x − x²)` with enumerated counts `1, 2, 5, 12, 29, 70, 169, 408` (first column 1) and `3, 7, 17, 41, 99, 239, 577, 1393` (free first column).
