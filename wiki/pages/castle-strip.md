...
- [[power-law-memory-rules](pages/power-law-memory-rules.md)] - the non-Markov generalization: replace `A(a, b)` on one adjacent pair with a Grunwald-Letnikov constraint reaching back over every prior column, K-truncate to an `h^{K-1}`-state 0/1 transfer matrix, and watch its Perron root ratchet through the reachable-field census as `K` grows.
- [[steep-polyominoes-q-motzkin-bessel](pages/steep-polyominoes-q-motzkin-bessel.md)] - the Motzkin-flavored strip (heights differ by ≤ 1) is a canonical steep-Dyck / q-Motzkin object; this is the polyomino-side counterpart of that strip sub-family.
