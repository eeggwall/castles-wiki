...
is a ring isomorphism. Equivalently, every system `r ≡ a_i (mod I_i)` has a solution. The map is always injective; comaximality is exactly what makes it surjective.[^1] For `R = Z` and `I_i = (n_i)` this is the integer CRT from number theory.
