...
- **Prove the `{0, 2, 7}` offset template** for the uniform-`w` set. Verified empirically through bracket 6 (`w <= 293`); the necessary condition `w in {1, 3, -1} (mod 9)` is proven for `k >= 3`, but the "smallest-offset representative" side of the sufficient condition (why `w = 19` is not uniform in bracket 3 even though `19 ≡ 1 mod 9`) still needs a general character-sum evaluation on `A_w, B_w mod 9`.
