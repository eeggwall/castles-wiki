...
**Structural conjecture**: `char_k mod 3` has repeated factors *only* at `k in {4, 6} (mod 12)`, always at multiplicity exactly 2, and always at the roots `+-1`. The recurrence `char_{k+1} = x^2 char_{k-1} - 2 char_k` reduces mod 3 to `char_{k+1} = x^2 char_{k-1} + char_k`; the pattern of repeated roots follows from the characteristic polynomial of this recurrence (in the `k` variable, at fixed `x = +-1`), which produces `y^2 - y - x^2 = 0`. At `x = 1` and `x = -1` mod 3, both give `y^2 - y - 1 = 0`, whose roots are the "silver-ratio-like" elements of `F_3` extensions, with a specific cycle length that reproduces the mod-12 pattern. A full proof that multiplicity stays `<= 2` for every `k` is left as the analytic follow-up.
...
- **v_3(per_h) = 1 for every h <= 101**: extended `char_k mod 3` factorization to `k = 100` gives max multiplicity 2 uniformly, hitting exactly at `k mod 12 in {4, 6}` with root `(x - 1)^2` or `(x + 1)^2`. No `k` produces multiplicity `>= 3`. Hence the mod-9 period contribution from any single `char_k` has `v_3 <= 1`, and `v_3(per_h) <= 1` for every row `h <= 101`.
...
- **Prove `char_k mod 3` multiplicity `<= 2` for all `k`**. Empirical through `k = 100`; the pattern `k mod 12 in {4, 6}` gives `(x - 1)^2` or `(x + 1)^2`, and no higher multiplicity occurs. A proof would follow from analyzing the `mod-3` recurrence `char_{k+1} = x^2 char_{k-1} + char_k` at `x = +-1` and using the resulting scalar recurrence `y^2 - y - 1 = 0` to bound repeated roots. If provable, `v_3(per_h) = 1` for *every* `h`, closing the row/column divisibility-gap theorem unconditionally.
