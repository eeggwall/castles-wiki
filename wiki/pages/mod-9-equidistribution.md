...
The offset set `{0, 2, 7}` is invariant from bracket 3 through bracket 6. Bracket 6 was verified by scanning offsets `0..49` (`50` values, `225` seconds with numpy-vectorized seed DP); only the three predicted values and one class-2 rate-matched-non-uniform at offset 45 (`w = 289`) appear. Bracket 2 is truncated (only offsets `0, 2` fit before the next bracket starts). This produces three explicit sequences of uniform `w`'s:
...
Equivalently, the uniform-`w` set is `{ 3^{k-1} + d : k >= 2, d in {1, 3, 8}, 3^{k-1} + d <= 3^k }`. **A mod-9 congruence reading**: for `k >= 3`, `3^{k-1} ≡ 0 (mod 9)`, so the uniform `w`'s satisfy `w ≡ 1, 3, or 8 (mod 9)` - or equivalently `w ≡ 1, 3, or -1 (mod 9)`. That is a necessary condition; the full template `w = 3^{k-1} + {1, 3, 8}` selects one representative per residue class per bracket (the smallest offset from `3^{k-1} + 1`), and higher-offset representatives of the same residue class (e.g. `w = 19 in bracket 3, w = 37 in bracket 4`) are *not* uniform.

**The coset-lift criterion (sufficiency, exact and computable).** Uniform `w` and non-uniform `w` are distinguished by a single condition on the shift-by-`per_w / 3` operator on `F(w, .) mod 9`:[^exec]

```
w uniform   iff   { F(w, h + per_w / 3) - F(w, h) mod 9 : h in [0, per_w) }  =  {3, 6}.
```

Every uniform `w` produces exactly the two-element image `{3, 6}` (the non-zero elements of `3 Z / 9`); every non-uniform `w` produces `{0, 3, 6}` - the extra `0` is a "coset-shift fixed point" `h` with `F(w, h + per_w / 3) = F(w, h) mod 9`, and its presence breaks uniformity. Verified across every `w in [10, 293]` with `w mod 9 in {1, 3, 8}` (17 uniform, 25 non-uniform); each uniform hit gives `{3, 6}` and each miss gives `{0, 3, 6}`, without a single exception.

This is the same coset-lift argument the `w = 4` blackboard proof (below) makes explicit: `F(4, h + 18) - F(4, h) = 6 (mod 9)` for even `h`, `3 (mod 9)` for odd `h`, both non-zero. For general uniform `w`, the shift `per_w / 3` plays the role of `18`, and the constant-difference-in-`{3, 6}` behavior generalizes.

The "smallest-offset representative" fact - that `w = 3^{k-1} + 1` is uniform but `w = 3^{k-1} + 10` (same mod-9 residue, offset `10`) is not - is now framed as: **why does the coset-lift image acquire `0` at higher offsets within a bracket?** Structurally, the shift-by-`per_w / 3` operator on the mod-9 state has eigenvalues that are cube roots of unity (since `(per_w / 3) * 3 = per_w` is the full state period). The `0` in the image corresponds to a fixed-point subspace of `F` under this operator, which is present at higher-offset `w` but empty at the three smallest offsets per bracket. A closed proof of "which `w` have empty fixed-point subspace" is the residual analytic item.
...
**Extended search: no multiplicity `>= 3` through `k <= 100`.** Direct factorization of `char_k mod 3` for `k = 1..100` produces:[^exec]

```
k with repeated mod-3 factor:  4, 6, 16, 18, 28, 30, 40, 42, 52, 54, 64, 66, 76, 78, 88, 90, 100.
```

These are exactly `k in [1, 100]` with `k mod 12 in {4, 6}`. In every case, the repeated factor is `(x - 1)^2` (for `k = 4 mod 12`) or `(x + 1)^2` (for `k = 6 mod 12`), and the multiplicity is always exactly `2`. **The global maximum multiplicity of any `char_k mod 3` factor for `k <= 100` is `2`.** So the mod-9 period contribution from any single repeated factor is exactly `3` (from `3^{ceil(log_3 2)} = 3`), never `9`. Combined with the fact that `ord_9(h), ord_9(h-1)` contribute at most one factor of 3, and LCM of two `v_3 = 1` quantities has `v_3 = 1`, this gives `v_3(per_h) <= 1` for every `h <= 101`.
...
**Proof.** The mod-3 recurrence `char_{k+1}(x) = x^2 char_{k-1}(x) + char_k(x)` evaluated at `x = 1` (respectively `x = -1`) reduces to the Fibonacci-like scalar recurrence `y_{k+1} = y_{k-1} + y_k` in `F_3`, together with the linked recurrences for the first and second derivatives:
...
(The `2 y_{k-1}` and `z_{k-1}` inhomogeneous terms come from differentiating `x^2 char_{k-1}` twice at `x_0 = +-1`. Both `x = 1` and `x = -1` produce the same recurrence because `x_0^2 = 1` and the derivatives of `x^2` at `x_0` are `2 x_0`, whose value mod 3 depends on the sign but not the structure of the recurrence.)
...
**Empirical extension (`k <= 200`).** No irreducible factor of `char_k mod 3` of degree `>= 2` has multiplicity `>= 2`. Combined with the proved linear-factor result, the global maximum multiplicity of any factor of `char_k mod 3` is `2` for every `k <= 200`. The conjecture "**degree-`>= 2` factors of `char_k mod 3` are always squarefree**" is the residual analytic item: if true, then `char_k mod 3` has max multiplicity `2` for every `k`, and by the multiplicity-inflation rule the mod-9 period of `char_k` has `v_3 <= 1` unconditionally. **This would close `v_3(per_h) = 1` for every `h` as a theorem**, promoting the divisibility gap from empirical to unconditional.

**Consequence for uniformity.** If `9 | per_w` (columns), the 9-bucket histogram *can* be exactly `[per_w / 9] * 9`, and empirically it is for the `{0, 2, 7}`-offset `w`'s. If `9 not | per_h` (rows), the 9-bucket histogram *cannot* be uniform, since some buckets hit `floor(per_h / 9)` and others `ceil(per_h / 9)`. Every row histogram carries an unavoidable `+-1`-per-residue rounding jitter, which sets a floor on the row-level equidistribution error at `1 / per_h`. That floor drops to zero as `h -> infinity` and `per_h -> infinity`, so equidistribution is still achieved in the limit, but no finite row ever hits `2/9` on the digit.
...
- Columns `w in {4, 6, 10, 12, 17, 28, 30, 35, 82, 84, 89, 244, 246, 251, ...}` have *exactly* rate `2/9` unconditionally - uniform histograms over their full h-period. The pattern is `w = 3^{k-1} + d` for `k >= 2, d in {1, 3, 8}` (with `d in {1, 3}` only in bracket 2), verified across brackets 2..6 (`w <= 293`). Necessary mod-9 congruence `w in {1, 3, -1} (mod 9)` derivable for `k >= 3`. **Exact sufficiency criterion (coset-lift test)**: `w` is uniform iff the shift-by-`per_w / 3` image on `F(w, .) mod 9` is exactly `{3, 6}` (never contains `0`); non-uniform `w`'s always have `0` in the image (a fixed-point of the coset shift), and this dichotomy is verified without exception across all 42 candidate `w`'s tested in brackets 3..6.
- **v_3(per_h) = 1 for every h <= 201**: extended `char_k mod 3` factorization to `k = 200` gives max multiplicity 2 uniformly, hitting exactly at `k mod 12 in {4, 6}` with root `(x - 1)^2` or `(x + 1)^2`. No `k` produces multiplicity `>= 3`. Hence the mod-9 period contribution from any single `char_k` has `v_3 <= 1`, and `v_3(per_h) <= 1` for every row `h <= 201`.
- **Linear-factor bound `(x +- 1)^3` never divides `char_k mod 3`, proved for all `k`**: the joint state `(char_k(x_0), char_k'(x_0), char_k''(x_0)) mod 3` at `x_0 = +-1` evolves in `F_3^6` with cycle length 24; enumeration over one full cycle shows the triple never simultaneously vanishes. This gives half of the "max multiplicity <= 2" claim unconditionally; only degree-`>= 2` irreducibles remain to be handled to close the divisibility gap as an unconditional theorem.
- **Convergence rate `alpha = 1/12`, with closed-form constant**: `deficit(N) = 2/9 - rate(N) ~ (4/54) * (4^{1/3} / 5^{1/4}) * N^{-1/12} = 0.0786 * N^{-1/12}`, matching observed values to 3 significant figures from `N = 10^{15}` onward. The exponent and constant come from the mixture argument: dominant column `w = 4` (uniform) contributes `~ N^{1/3}` cells at exact `2/9`; the leading deficit column `w = 5` (rate `8/54`, deficit `4/54`) contributes `~ N^{1/4}` cells, ratio `N^{-1/12}`.
...
- **Structural proof of the coset-lift criterion**. The sufficiency test `shift-by-per_w/3 image = {3, 6}` is exact and computable, but the closed algebraic reason it succeeds at exactly the `{0, 2, 7}` offsets per bracket - equivalent to "the fixed-point subspace of the shift-by-`per_w / 3` operator on `F(w, .) mod 9` is trivial exactly for those three `w` per bracket" - is open. Framed as: which `w` make the mod-9 reduction of `F(w, .)` orthogonal to the eigenvalue-1 eigenspace of the shift-by-`per_w/3` operator.
- **Prove degree-`>= 2` irreducibles of `char_k mod 3` are squarefree**, closing the divisibility-gap theorem unconditionally. Empirical through `k <= 200`. The linear-factor half of the multiplicity-2 bound is now proved (joint-state cycle argument at `x = +-1`); what remains is showing that no `char_k mod 3` has an irreducible factor of degree `>= 2` at multiplicity `>= 2`. Equivalent to: `gcd(char_k, char_k') mod 3` is always a product of `(x - 1)` and `(x + 1)` factors, never contains a higher-degree irreducible.
