...
The work is organized into **veins**, each a thread from the castle out to a family of sequences.[^2] The numbering follows the raw `mine-notes.md` in `raw/oeis-pe502/`, whose exploratory index went 1, 2, 3, 4, 7, 8, 9, 9b - two entries (5 and 6) were reserved in an earlier plan for threads that folded into their neighbours (the tower / block-count material became Vein 7; the height-2 hyperbolic material sits inside Vein 3) and the gaps were kept so the raw citations remain stable.[^2]
...
[^2]: [[oeis-mining-pe502](pages/oeis-mining-pe502.md)] `mine-notes.md` §§"Vein 1", "Vein 2", "Vein 3", "Vein 4", "Vein 7", "Vein 8", "Vein 9", "Vein 9b" L19-150 [synthesis] - the eight veins present in the raw notes (numbering 1-4, then 7-9b; 5 and 6 were reserved in the plan and their content folded into the retained veins, see the paragraph above).
