...
**As a functional equation.** Let `S(q, z)` be the signed castle GF, `sum χ(C) q^area(C) z^width(C)`. A nontrivial prime `(1, X, 1)` is `X = Y` raised by one full row. That adds `width(Y)` to the area and one block, so `χ(X) = -χ(Y)`, and `P_s = q (1 - S(q, q))`. The identity becomes

```
1 + q + q S(q, 1)  =  1 / (1 - q + q S(q, q))
```

This is the signed version of the unsigned equation on [[prime-castles](pages/prime-castles.md)], and it was checked coefficient by coefficient through `q^17`.[^1] Unlike the unsigned case, neither side is known in closed form. The equation ties the area-only signed count `S(q, 1)` to the width-refined one at `z = q`, the same shape as the `q = 1` shadow problem on [[q-differential-system](pages/q-differential-system.md)] (own reasoning).
...
**What the characters buy.** Any list of characters weights the primes and passes through `1/(1 - P)`. The joint GF of area, block sign and peaks is

```
1/(1 - P(q, t)),    P(q, t) = q (1 - t S(q, q))      (t marking peaks)
```

a one-line refinement of the identity above (own reasoning; checked only at `t = 1`). It gives the area × peaks bistatistic with the sign built in.
...
- A closed form, or a q-series in the style of [[convex-polyomino-by-area](pages/convex-polyomino-by-area.md)], for `S(q, 1)` from the functional equation. The unsigned equation is trivial because `A(q, q)` is rational. The signed `S(q, q)` is not known to be.
- The growth of `odd - even` by area. The terms `..., 44, -98, 136, -230` grow much more slowly than `2^n`, the area-graded version of "the parity is a lower-order correction" on [[castle-perimeter](pages/castle-perimeter.md)].
...
[^1]: Checked by execution, 2026-09-22 (block above, plus the functional equation through `q^17`), over every composition of `n ≤ 16`. The `even(n)` values reproduce raw/oeis-pe502/vein9-area.md §"Parity splits" L46-47 - "`even(n)` | 0, 1, 2, 5, 8, 17, 30, 65, 122, 261, 502, 1043 ... `odd(n)` | 1, 1, 2, 3, 8, 15, 34, 63, 134, 251, 522, 1005".
