...
2. **A146559**: the `a(n) = P(1, n − 1)` comment is still unsubmitted, but the derived `A146559 = A038503 − A038505` identity is now in OEIS via the A038503 formula `a(n) = A038505(n) + A146559(n)` ([[signed-tower-count](pages/signed-tower-count.md)]).
