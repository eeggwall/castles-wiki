...
**In the code.** On the rational-function path the extractor is the `extractCoeff` switch: `extractCoeffDirect` (a direct power-series expansion, unrolled by 8, with `int[]` coefficients — safe because `p < 2^30`, so eight products below `p^2` stay under `Long.MAX_VALUE`) for small `w`, or `extractCoeffMatExp` + `kitamasa` + `polyMulMod` otherwise, running Kitamasa on the characteristic polynomial `x^d − ∑ rec[i] x^{d−1−i}` built from `−den[1..D]`.[^5] Matrix helpers `matmul`/`matpow` exist for the single-value variant but are unused on the composite path, which uses Kitamasa throughout.[^6]
...
- [[project-euler-502-implementation-notes](pages/project-euler-502-implementation-notes.md)] — the `extractCoeff` switch, `extractCoeffDirect` numerics, and the charpoly construction from `−den[1..D]`.
...
[^5]: [[project-euler-502-implementation-notes](pages/project-euler-502-implementation-notes.md)] §"Rational-function path" L39-41 — "extractCoeff - the direct-vs-Kitamasa switch. extractCoeffDirect - unrolled by 8 ... cast to int[] since p < 2^30 and 8 products of < p^2 stay under Long.MAX_VALUE. extractCoeffMatExp + kitamasa + polyMulMod - standard Kitamasa on the characteristic polynomial x^d - ∑ rec[i] x^{d-1-i} derived from -den[1..D]."
