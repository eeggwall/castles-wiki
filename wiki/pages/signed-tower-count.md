...
`A146559` is a proposed cross-link target: `a(n) = P(1,n−1)`, the real part of `(1+i)^n` read as a signed castle count, with the genuinely new formula `A146559(n) = A038503(n) − A038505(n)` tying the signed vein to the height-2 [[hyperbolic-sequence-family](pages/hyperbolic-sequence-family.md)].[^3] The `P(k,·)` families for `k ≥ 2` generalize A146559; the even-`k`, all-positive rows are the cleanest new-sequence candidates and are **confirmed novel** — `P(2,·): 1,1,3,9,19,33,59,121,259,529,…` (order 3) and `P(4,·): 1,1,5,25,85,225,541,1385,3973,…` (order 5) both return no OEIS match (searched 2026-09-18, [[oeis-index](pages/oeis-index.md)]).[^4] The whole family has a Pell/Chebyshev closed-form denominator (roots `−x ± √(x²+1)`), worked out on [[generating-function-gallery](pages/generating-function-gallery.md)].
