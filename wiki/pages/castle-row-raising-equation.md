...
[^4]: [[prellberg-brak-1995-cluster-models](pages/prellberg-brak-1995-cluster-models.md)] `prellberg-brak-1995-partially-directed-cluster-models.txt` §4 L673-682 - "the functional equations for bar-graph polygons, alternating bar-graph polygons, staircase polygons, and ZL walks can all be linearized, as they are of the form G(x)G(qx) + a(x)G(x) + b(x)G(qx) + c(x) = 0 (4.1) ... by use of the transformation G(x) = α H(qx)/H(x) - b(x) (4.2)". Checked by execution, 2026-09-23 (sympy): for both equations, substituting (4.2) and multiplying by `H(x)` gives their (4.3), `α² H(q²x) + α[a(x) - b(qx)] H(qx) + [c(x) - a(x) b(x)] H(x) = 0`, identically.
[^5]: [[prellberg-brak-1995-cluster-models](pages/prellberg-brak-1995-cluster-models.md)] `prellberg-brak-1995-partially-directed-cluster-models.txt` §3.1 L401-402, L468-473 - "bar-graph polygons, that is, column-convex polygons with a horizontal lower boundary"; "we get the functional equation B(x,y,q) = B(qx,y,q) y + {1 + B(qx,y,q)} qx {y + B(x,y,q)} (3.11)". Computed by execution, 2026-09-23: (3.11) iterated in sympy through area 7; the area-3 coefficient is `x³y + 2x²y² + xy³`, `x = y = 1` gives `1, 2, 4, …, 64`, and with `E = 1 + B(z, -1, q)` the signed row-raising equation holds with zero residual through `q^7`.
