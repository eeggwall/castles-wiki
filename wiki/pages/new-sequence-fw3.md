...
- **`x² − 2x + 2 = char_1`** (roots `1 ± i`) is `P(1,·)`, and **`(x−2)(x² − x + 2) = char_2`** is `P(2,·)` ([[signed-tower-count](pages/signed-tower-count.md)]). `char_2` is also the modulus of the toy castle cryptosystem: [[castle-cryptography-ring](pages/castle-cryptography-ring.md)] builds the ring `F_p[x]/(char_2)` and finds its period `3400` mod `101` from exactly these factors; [[castle-cryptography-number-theory](pages/castle-cryptography-number-theory.md)] explains why the reducible `char_2` splits the discrete log by CRT; [[castle-cryptography-round-two](pages/castle-cryptography-round-two.md)] traces linear-complexity deficits to the root coincidence `λλ̄ = 2` of `x² − x + 2` - the same shared root `2` that drops this sequence's order from 7 to 6. Why `char_2` factors at all is on [[tower-parity-sectors](pages/tower-parity-sectors.md)].
