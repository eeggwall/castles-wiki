...
sources: [analytic-combinatorics-ch1-ogfs, column-convex-polygon-enumeration, counting-horizontally-convex-polyominoes, castle-by-area, bender-1974-convex-n-ominoes]
created: 2026-09-22
updated: 2026-09-22
---
...
| directed convex, convex | two boundaries, two phases each | not derived here. Bender gives only the three-piece decomposition and its asymptotics (see "One growth constant"); the source for the exact series is unread (see "Sources not yet read") [^7] |

The slice state grows with the family: none for bar graphs, the last height for Ferrers and stacks, and a slice length with left and right offsets for parallelograms. For convex polyominoes it is the whole last column plus a phase bit for each boundary. The column sweep below is this construction with the state written out explicitly.
...
The prefactors are `0.29745...` for parallelograms and `2.91959...` for convex polyominoes.[^8] Bender ([[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)]) obtained `mu = 2.30913859330` as the reciprocal of the first root of `P_2(x, 1, 1) = 1`, and `0.29745 mu^n` for parallelograms from that simple pole.[^15] Flajolet and Sedgewick follow his analysis in Example IX.14.[^6][^14] The digits printed there end `...331230`, which OEIS flags as wrong; the value above is the corrected one, and it agrees with Bender's.[^8] Bender's convex prefactor is the exception. He prints `2.67564` in his abstract and `2.67483` in his equation (11), and both are about 8% below `2.91959`, which the column sweep confirms: `c(n)/mu^n = 2.91962` at `n = 40`.[^16]

Bender also explains why the convex rung shares `mu` with the parallelogram rung. Cutting a convex polyomino at the last row where its left end moves left and the first row where its right end moves right leaves a trapezoid, a parallelogram and an inverted trapezoid. The trapezoid series has radius of convergence 1, so the parallelogram carries all the exponential growth.[^17] His trapezoid series at `y = 1` is the stack series, the third rung of the ladder (own check through area 20), so the convex rung is two stack rungs glued to the ends of a parallelogram rung. Directed convex polyominoes sit between the two (parallelogram ⊂ directed convex ⊂ convex), so their growth rate is squeezed to the same `mu`. Numerically the parallelogram ratio `a(n+1)/a(n)` is already `2.30914` at `n = 25`, while the directed-convex (`2.30924`) and convex (`2.3093`) ratios approach it more slowly.[^1]
...
These appear only in OEIS link lists and were not read. Nothing on this page rests on them; they are leads for the next ingest.
...
[^7]: https://oeis.org/A067675 and https://oeis.org/A067676 (fetched 2026-09-22) - both entries list "M. Bousquet-Mélou and J.-M. Fédou, The generating function of convex polyominoes: the resolution of a q-differential system, Discrete Math. 137 (1995) 53-75" in their links. Only the title was seen; the paper has not been read, and no claim on this page rests on it.
...
[^15]: [[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)] pp.221-222 §3 - "γ = r^{-1}, where r is the radius of convergence of P(x) ... it has a singularity at x = r which ... is a simple pole due to a simple root of P_2(x, 1, 1) = 1 ... We found r^{-1} = γ = 2.30913859330 by using the first ten terms in (5b) ... p(n) ~ 0.29745γ^n since the pole is simple."
[^16]: [[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)] p.219 abstract and p.224 eq. (11) - "c(n) ~ fγ^n, where γ = 2.30914 and f = 2.67564"; "f = 2K^2/(rP_1(r, 1, 1, 1)P_2'(r, 1, 1)) = 2.67483". The `n = 40` ratio is from the column sweep on this page, run 2026-09-22: `c(40) = 1007688926337136`, and `c(n)/mu^n` falls monotonically from 2.92061 at `n = 26` to 2.91962 at `n = 40`.
[^17]: [[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)] pp.222-223 §4 and Fig. 1 - "The three parts of the convex animal are determined by those l_i and r_i with i in the ranges i ≤ I, I < i < J, i ≥ J (see Fig. 1) ... Since T(x, 1) has radius of convergence 1 which exceeds r, it follows that c(n) ~ 2c*(n)."
