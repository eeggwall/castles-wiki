...
Klarner and Rivest proved that the convex and parallelogram limits exist and coincide: every convex polyomino cuts into a stack, a parallelogram and an inverted stack, so `p(n) <= c(n) <= (n+2)^4 p(n)`. They then bracketed the common limit to `2.309138...` by truncating the parallelogram row-to-row kernel `min{m, n}` from both sides.[^16] Bender obtained the same constant analytically, as the reciprocal of the first root of `P_2(x, 1, 1) = 1`, and derived `0.29745 mu^n` for parallelograms from that simple pole.[^17] The prefactors are `0.29745...` for parallelograms and `2.91959...` for convex polyominoes.[^8] For directed convex polyominoes it is `0.6589555418`, from the Bousquet-Mélou-Fédou series at area 120. OEIS A067676 does not list it. The same computation reproduces `2.9195985097` for convex, while the Bousquet-Mélou-Fédou paper itself prints `C ≃ 2.67564`, a value its own generating function does not give.[^15] Bender's convex prefactor differs too: he prints `2.67564` in his abstract and `2.67483` in his equation (11), both about 8% below `2.91959`, which the column sweep confirms at `c(n)/mu^n = 2.91962` at `n = 40`.[^17] Bender's abstract value is the one Delest and Viennot and Bousquet-Mélou and Fédou later quote.[^19] Flajolet and Sedgewick follow Bender's analysis in Example IX.14.[^6][^14] The printed digits there end `...331230`, which OEIS flags as wrong; the value above is the corrected one.[^8] Bender also explains why the convex rung shares `mu` with the parallelogram rung: cutting a convex polyomino at the last row where its left end moves left and the first row where its right end moves right leaves a trapezoid, a parallelogram and an inverted trapezoid, and the trapezoid series has radius of convergence 1, so the parallelogram carries all the exponential growth.[^18] His trapezoid series at `y = 1` is the stack series, the third rung of the ladder (own check through area 20), so the convex rung is two stack rungs glued to the ends of a parallelogram rung. Directed convex polyominoes sit between the two (parallelogram ⊂ directed convex ⊂ convex), so their growth rate is squeezed to the same `mu`. It is also visible in the formulas: all three generating functions are quotients over the same `J_0`, so the zero of `J_0` sets their common growth rate.[^7] Numerically the parallelogram ratio `a(n+1)/a(n)` is already `2.30914` at `n = 25`, while the directed-convex (`2.30924`) and convex (`2.3093`) ratios approach it more slowly.[^1]
...
[^17]: [[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)] pp.221-222 §3 and p.219 abstract, p.224 eq. (11) - "γ = r^{-1}, where r is the radius of convergence of P(x) ... it has a singularity at x = r which ... is a simple pole due to a simple root of P_2(x, 1, 1) = 1 ... We found r^{-1} = γ = 2.30913859330 by using the first ten terms in (5b) ... p(n) ~ 0.29745γ^n since the pole is simple"; "c(n) ~ fγ^n, where γ = 2.30914 and f = 2.67564"; "f = 2K^2/(rP_1(r, 1, 1, 1)P_2'(r, 1, 1)) = 2.67483". The `n = 40` ratio is from the column sweep on this page, run 2026-09-22: `c(40) = 1007688926337136`, and `c(n)/mu^n` falls monotonically from 2.92061 at `n = 26` to 2.91962 at `n = 40`.
[^18]: [[bender-1974-convex-n-ominoes](pages/bender-1974-convex-n-ominoes.md)] pp.222-223 §4 and Fig. 1 - "The three parts of the convex animal are determined by those l_i and r_i with i in the ranges i ≤ I, I < i < J, i ≥ J (see Fig. 1) ... Since T(x, 1) has radius of convergence 1 which exceeds r, it follows that c(n) ~ 2c*(n)."
[^19]: [[algebraic-languages-and-polyominoes-enumeration](pages/algebraic-languages-and-polyominoes-enumeration.md)] p.170 §1 - "Klarner and Rivest [21] and Bender [1] gave asymptotic estimates for the number a_n of convex polyominoes having area n. More precisely, a_n ~ Γγ^n with γ = 2.30914... and Γ = 2.67564..." (OCR normalized; [21] is Klarner-Rivest 1974, [1] is Bender 1974); [[bousquet-melou-fedou-1995-convex-polyominoes](pages/bousquet-melou-fedou-1995-convex-polyominoes.md)] p.59 L305-308 - "C ≃ 2.67564 and a ≃ 2.30914".
