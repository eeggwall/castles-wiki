...
category: Sources
summary: The tower (heap-of-pieces) block-count g.f. is Narayana_w(x)/(1−x)^w; width rows are A005408/A005891/A063490/A160747 (w≥6 new) — the Catalan/Narayana thread, living in the tower not the convex count.
...
T(w,b) = Σ_{k=1..w} N(w,k) · C(b + w − k, w − 1),
   g.f. by b:  Narayana_w(x) / (1 − x)^w,   Narayana_w(x) = Σ_k N(w,k) x^{k−1}
```
...
- **A001263** (Narayana) — add a Formula: the tower block-count g.f. has the Narayana polynomial as numerator, `T(w,b) = Σ_k N(w,k) C(b+w−k, w−1)`. Stated as a generating-function identity, **not** a peaks bijection — a direct peaks refinement was tested and does not factor this way.[^3]
...
- Tower block-count g.f. `= Narayana_w(x)/(1−x)^w`; `T(w,b) = Σ_k N(w,k) C(b+w−k, w−1)`, verified `w=1..7`.[^1]
...
- [[narayana-numbers](pages/narayana-numbers.md)] — A001263, the g.f. numerator.
...
[^1]: [[tower-narayana-polynomial](pages/tower-narayana-polynomial.md)] `crosslink-avenues.md` §"Tier 2" L36-47 — "T(w,b) = # towers of width w with exactly b blocks = sum_{k=1..w} Narayana(w,k) * C(b + w − k, w − 1) ... g.f. = (Narayana_w(x)) / (1 − x)^w ... verified for w = 1..7 against brute force ... Narayana(w,k) = ... = A001263."
