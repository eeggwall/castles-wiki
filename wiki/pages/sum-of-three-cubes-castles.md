...
So the width-3 even counts at odd height are the **triangular numbers** `1, 3, 10, 21, 36, 55, ...` (A000217), and at even height the "pentagonal-like" `6, 31, 76, 141, 226, 331, ...`. Triangular numbers are `0, 1, 3, 6 (mod 9)` and never `4, 5`, so every triangular castle count passes the residue test; `5 C(h,2) + 1` is `4 (mod 9)` iff `C(h,2) = 6 (mod 9)` iff `h = 4, 6 (mod 9)`, and is never `5`. Therefore
