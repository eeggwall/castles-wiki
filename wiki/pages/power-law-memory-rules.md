...
The F Department opens a wider question: what if the rule reads *every* prior column, with weights decaying as a power of the distance? The natural candidate is the **Grunwald-Letnikov fractional-difference kernel**, `(-1)^k C(alpha, k)`, which decays like `k^{-alpha - 1}` for large `k` ([[fractional-block-count](pages/fractional-block-count.md)]). The fractional analog of the "1-smooth" rule `|c_n - c_{n-1}| <= 1` (Motzkin, alpha = 1) is then the **fractional 1-smooth rule**
...
## The N Department verdict

The IDEAS.md item asks: is the K -> infinity growth constant a new number for the N Department, or does it land in the [[reachable-field-census](pages/reachable-field-census.md)] after all? Two answers, both with the same negative sign.
...
- The K -> infinity limit at `h = 2` for `alpha in (0, 1)` appears to be `2`, the unrestricted binary-strip growth, an integer already in the census. No new N Department transcendental falls out.
...
- The NTT / F_65537 acceleration proposed in the IDEAS.md item accelerates per-skyline evaluation of the GL sum, not the transfer-matrix count; it belongs on the [[fractional-block-count](pages/fractional-block-count.md)] side of the F Department, not here.
...
- **Threshold `T < 1`.** All numerics here use threshold `T = 1` (chosen to recover Markov 1-smooth at `alpha = 1`). Tightening to `T = 1/2` gives a different problem whose K -> infinity limit could be strictly less than `2` and whose algebraic status is unknown. This is where the "new number for the N Department" question actually lives.
...
- [[fractional-block-count](pages/fractional-block-count.md)] - the F Department's block-count-side fractional operator on skylines; the per-skyline evaluation of the GL sum this page uses is the same primitive.
- [[fractional-width-and-height](pages/fractional-width-and-height.md)] - the F Department's coordinate-side fractional operator; different axis, same GL kernel.
...
- [[levy-flights](pages/levy-flights.md)] - the F Department sibling: Riascos-Mateos `L^α` on the castle-graph Laplacian, the same fractional-calculus wall approached from the spectral side rather than the strip-rule side.
...
- [[reachable-field-census](pages/reachable-field-census.md)] - the algebraic universe the N Department question runs against; this page argues `K -> infinity` at `h = 2` stays on integer `2` rather than reaching a new transcendental.
