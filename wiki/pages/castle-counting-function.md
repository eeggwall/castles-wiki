...
**These counts appear in the OEIS.** The [[oeis-mining-pe502](pages/oeis-mining-pe502.md)] pass connected the castle counts to existing sequences: the any-parity count `A(w,h) = h^w − (h−1)^w` is the difference-of-powers family (`A000225`, `A001047`, …); the height-2 even/odd counts are the order-4 [[hyperbolic-sequence-family](pages/hyperbolic-sequence-family.md)] (`F(w,2) = A038505(w+1)`, `odd(w,2) = A038503(w+1)−1`; submitted to OEIS 2026-09-18 — see [[oeis-height2-hyperbolic-castles](pages/oeis-height2-hyperbolic-castles.md)]); and by area the convex count is `A001523` ([[weakly-unimodal-composition](pages/weakly-unimodal-composition.md)]). The `h ≥ 3` rows are new C-finite sequences — [[new-sequence-fw3](pages/new-sequence-fw3.md)] is the drafted candidate `F(w,3)`.[^7]
