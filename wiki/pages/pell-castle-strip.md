...
A castle strip is a skyline read left to right under a neighbor rule, and the rule is an `h × h` 0/1 transfer matrix whose states are the column heights ([[castle-strip](pages/castle-strip.md)]). Take the **1-smooth** rule on heights `{1, 2, 3}`: adjacent columns differ in height by at most 1 (`|c_{i+1} − c_i| ≤ 1`, the Motzkin-flavoured Axis-2 predicate of [[castle-classification](pages/castle-classification.md)]). Its transfer matrix and characteristic polynomial are
